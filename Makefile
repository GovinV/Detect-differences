...
test-avec-valgrind: all
	VALGRIND="valgrind -q --trace-children=yes" ; export VALGRIND ; for i in test/test-*.sh ; do echo $$i ; sh $$i || exit 1 ; done

couverture-et-tests: clean coverage test-avec-valgrind gcov
